...
function max_dt(u, t,
                mesh::Union{StructuredMesh{2}, UnstructuredMesh2D, P4estMesh{2},
                            T8codeMesh{2}},
                constant_speed::False, equations::AbstractEquations{2}, dg::DG, cache)
    # to avoid a division by zero if the speed vanishes everywhere,
    # e.g. for steady-state linear advection
    max_scaled_speed = nextfloat(zero(t))
...
function max_dt(u, t,
    ...
    for element in eachelement(dg, cache)
        max_lambda1 = max_lambda2 = zero(max_scaled_speed) #max_lambda3
        for j in eachnode(dg), i in eachnode(dg)
            u_node = get_node_vars(u, equations, dg, i, j, element)
        ...
        #println(max_lambda1, ", ", max_lambda2)
        max_scaled_speed = max(max_scaled_speed, max_lambda1 + max_lambda2)
    end
