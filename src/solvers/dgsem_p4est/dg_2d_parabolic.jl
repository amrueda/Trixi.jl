...
function create_cache_parabolic(mesh::P4estMesh{2}, equations_hyperbolic::AbstractEquations,
    ...
    if ndims(equations_hyperbolic) == 2
        viscous_container = init_viscous_container_2d(nvariables(equations_hyperbolic),
                                                    nnodes(dg.basis), nelements(elements),
                                                    uEltype)
    elseif ndims(equations_hyperbolic) == 3
        viscous_container = init_viscous_container_2d_eq_3D(nvariables(equations_hyperbolic),
                                                            nnodes(dg.basis), nelements(elements),
                                                            uEltype)
    end
...
# Calculate gradient for 2D p4est meshes with 2D equations
function calc_gradient!(gradients, u_transformed, t,
                        mesh::P4estMesh{2}, equations_parabolic::AbstractEquationsParabolic{2},
...
# Calculate gradient for 2D p4est meshes with 3D equations
function calc_gradient!(gradients, u_transformed, t,
                        mesh::P4estMesh{2}, equations_parabolic::AbstractEquationsParabolic{3},
                        boundary_conditions_parabolic, dg::DG,
                        cache, cache_parabolic)
        ...
        @threaded for element in eachelement(dg, cache)
            for l in eachnode(dg)
                for v in eachvariable(equations_parabolic)
                    ...
                    # Compute z-component of gradients

                    # surface at -x
                    _, _, normal_direction_z = get_normal_direction(1, contravariant_vectors,
                                                                    1, l, element)
                    gradients_z[v, 1, l, element] = (gradients_z[v, 1, l, element] +
                                                     surface_flux_values[v, l, 1, element] *
                                                     factor_1 * normal_direction_z)

                    # surface at +x
                    _, _, normal_direction_z = get_normal_direction(2, contravariant_vectors,
                                                                 nnodes(dg), l, element)
                    gradients_z[v, nnodes(dg), l, element] = (gradients_z[v, nnodes(dg), l,
                                                                          element] +
                                                              surface_flux_values[v, l, 2,
                                                                                  element] *
                                                              factor_2 * normal_direction_z)

                    # surface at -y
                    _, _, normal_direction_z = get_normal_direction(3, contravariant_vectors,
                                                                 l, 1, element)
                    gradients_z[v, l, 1, element] = (gradients_z[v, l, 1, element] +
                                                     surface_flux_values[v, l, 3, element] *
                                                     factor_1 * normal_direction_z)

                    # surface at +y
                    _, _, normal_direction_z = get_normal_direction(4, contravariant_vectors,
                                                                 l, nnodes(dg), element)
                    gradients_z[v, l, nnodes(dg), element] = (gradients_z[v, l, nnodes(dg),
                                                                          element] +
                                                              surface_flux_values[v, l, 4,
                                                                                  element] *
                                                              factor_2 * normal_direction_z)
...
# of a 2D equation on a 2D p4est mesh
...
# This is the version used when calculating the divergence of the viscous fluxes
# of a 3D equation on a 2D p4est mesh
function calc_volume_integral!(du, flux_viscous,
                               mesh::P4estMesh{2},
    ...
    @threaded for element in eachelement(dg, cache)
        ...
        for j in eachnode(dg), i in eachnode(dg)
            flux1 = get_node_vars(flux_viscous_x, equations_parabolic, dg, i, j, element)
            flux2 = get_node_vars(flux_viscous_y, equations_parabolic, dg, i, j, element)
            flux3 = get_node_vars(flux_viscous_z, equations_parabolic, dg, i, j, element)
