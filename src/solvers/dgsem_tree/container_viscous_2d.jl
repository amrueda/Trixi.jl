...
# Container for solving a 3D equation in a 2D grid
mutable struct ViscousContainer2DEq3D{uEltype <: Real}
    u_transformed::Array{uEltype, 4}
    # Using an outer fixed-size datastructure leads to nasty implementations,
    # see https://github.com/trixi-framework/Trixi.jl/pull/1629#discussion_r1355293953.
    # Also: This does not result in speed up compared to using tuples for the internal 
    ...
    function ViscousContainer2DEq3D{uEltype}(n_vars::Integer, n_nodes::Integer,
                                             n_elements::Integer) where {uEltype <: Real}
        new(Array{uEltype, 4}(undef, n_vars, n_nodes, n_nodes, n_elements),
            [Array{uEltype, 4}(undef, n_vars, n_nodes, n_nodes, n_elements) for _ in 1:3],
            [Array{uEltype, 4}(undef, n_vars, n_nodes, n_nodes, n_elements) for _ in 1:3],
            Vector{uEltype}(undef, n_vars * n_nodes^2 * n_elements),
            (Vector{uEltype}(undef, n_vars * n_nodes^2 * n_elements),
             Vector{uEltype}(undef, n_vars * n_nodes^2 * n_elements),
...
function init_viscous_container_2d_eq_3D(n_vars::Integer, n_nodes::Integer,
                                         n_elements::Integer,
                                   ::Type{uEltype}) where {uEltype <: Real}
    return ViscousContainer2DEq3D{uEltype}(n_vars, n_nodes, n_elements)
end
...
# `unsafe_wrap`ping multi-dimensional `Array`s around the
# internal storage.
# TODO: This resize can be merged with the one for ViscousContainer2D.
function Base.resize!(viscous_container::ViscousContainer2DEq3D, equations, dg, cache)
    capacity = nvariables(equations) * nnodes(dg) * nnodes(dg) * nelements(dg, cache)
    resize!(viscous_container._u_transformed, capacity)
    for dim in 1:3
        resize!(viscous_container._gradients[dim], capacity)
